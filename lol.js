...
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=62895377298173:+62895377298173\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
async function starts() {
 ...
 if (isCmd) cmdadd()
		...
		if (!isRegistered) return reply(ind.noregis())
                ...
                lolteam.sendMessage(from, 'Tuh kontak Mr.Devils <•_•> ,Jangan Diubah Ya Kontaknya (*>*)',MessageType.text, { quoted: lol} )
	        ini_buffer = await getBuffer("https://i.ibb.co/tqX1xmn/20210326-134340.jpg")
                lolteam.sendMessage(from, ini_buffer, MessageType.image, {quoted: lol, caption: '*Join Grup Bot Wa*/n*https://chat.whatsapp.com/Ie5vic3gY4ZE9MMV0ysa2F*/n/n*Grup bot telegram :*/n*@botindonesia*'})
                 break
				case 'addprem':
				if (!isRegistered) return reply(ind.noregis())
					if (sender.split("@")[0] != owner) return reply("Command only for owner bot")
    ...
    case 'ttp':
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK cowner')
                    ...
                    teks = args.join(" ")
                    buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/attp?apikey=${apikey}&text=${teks}`)
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    teks = args.join(" ")
                    buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/nulis?apikey=${apikey}&text=${teks}`)
                    lolteam.sendMessage(from, buffer, image, { quoted: lol})
                    break
                    case 'apikeycek':
		if (!isRegistered) return reply(ind.noregis())
		...
		username = args[0]
		comment = args[2]
                    buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/ytcomment?apikey=${apikey}&username=${username}&comment=${comment}&img=https://i.ibb.co/JdfQ73m/photo-2021-02-05-10-13-39.jpg`)
                    lolteam.sendMessage(from, buffer, image, { quoted: lol})
                    break
                    case 'phkomen':
		if (!isRegistered) return reply(ind.noregis())
		...
		username = args[0]
		comment = args[2]
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/phcomment?apikey=${apikey}&img=https://i.ibb.co/JdfQ73m/photo-2021-02-05-10-13-39.jpg&text=${comment}&username=${username}`)
                    lolteam.sendMessage(from, buffer, image, { quoted: lol})
                    break
                    case 'amongus':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nExample: ${prefix + command} MrA43G`)
                    buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/amongus?apikey=${apikey}&text=${body.slice(9)}`)
                    lolteam.sendMessage(from, buffer, sticker, { quoted: lol})
                    break
                    case 'tolol': 
			if (!isRegistered) return reply(ind.noregis())
				...
				gatauda = body.slice(6)
				buffer = await getBuffer(`http://api.lolhuman.xyz/api/smoji/${gatauda}?apikey=${apikey}`, {method: 'get'})
				lolteam.sendMessage(from, buffer, image, {quoted: lol})
				break
				case 'quotemaker':
			if (!isRegistered) return reply(ind.noregis())
